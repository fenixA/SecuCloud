...
/*
     FILE ARCHIVED ON 14:32:22 Jan 7, 2010 AND RETRIEVED FROM THE
...
OnlineOpinion.ocode = function(name) {
	...
	*/
	...
	this.launchCC = function () {
		try {
			...
			if(this.Cookie.matchurl(this.Metrics.core.referer, this.Preferences.Persistence.cookie_type)!=1){
				if (this.Preferences.Plugins.CardOnPage.enabled == true) {
					document.getElementById(this.Preferences.Plugins.CardOnPage.iframe_id).src = '/web/20100107143222/https://secure.opinionlab.com/ccc01/comment_card.asp?'+(this.Preferences.Render.type=='asm'?'asm=1&':'')+'static=1&' + this.serialize(this.Metrics.core, 0) + '&custom_var=' + this.serialize(this.Metrics.custom, 1);
  ...
  // Method for resetting the Referer, and have it properly rewritten if needed.
	...
	this.post = function () { // Used for onPage static card posting of data
		...
		JSONP.src = "/web/20100107143222/https://secure.opinionlab.com/rate36s.asp?"+odata+"&"+cdata+"&signature="+osignature;
