...
Hinzu kommt die eine Einschätzung ob und wie eine solche Applikation marktfähig entwickelt und vertrieben werden kann.

\subsection{Gang der Arbeit}
Zunächst werden im Kapitel \fullref{GrundlagenV} alle Grundlagen erläutert, die zum Verständnis der in \fullref{UmsetzungV}  beschriebenen Konzeptionierung und Implementierung dienen. Darin werden die verwendeten Sicherheitsalgorithmen, die Notwendigkeit von Cloudcomputing, dessen wirtschafliche Aspekte, die Abgrenzung der einzelnen Anbieter, die Programmiersprache Java, das Kryptosystem AES sowie die zur Entwicklung der Applikation verwendeten Werkzeuge und Technologien beschrieben. Nachfolgen wird in \fullref{ErgebnisseV} die Applikation selbst in Architektur und Aufbau des Programmcodes und das entworfene Sicherheitskonzept erläutert. Außerdem findet sich hier auch eine detaillierte Beschreibung des graphischen Interfaces und eine Bedienungsanleitung. In \fullref{DiskussionV} werden die Ergebnisse (die Applikation) im Sinne der Motivation und der Zielsetzung bewertet, sowie Möglichkeiten zur Optimierung der Applikation und eine Aussicht auf zukünftige Weiterentwicklungen gegeben.

In der Ausarbeitung werden teilweise Anglizismen und englische Begriffe in deutssprachingen Fließtexten verwendet, zum Teil in Eindeutschungen. Dies geschieht, da das Zielpuplikum eine informatikinteressierte Fachöffentlichkeit ist. Wir gehen davon aus, dass viele Begriffe der Informatik auch im deutschen Sprachgebrauch auf immer englisch verwendet werden und so in diesem Zusammenhang eindeutiger sind als ihre deutsche Übersetzung. So ist zum Beispiel ein Bytearray ein stehender Begriff, während die Übersetzung Bytefeld einer Erläuterung bedürfen würde.
...
\begin{minipage}{\linewidth}
	\centering
	\includegraphics[width=1.0\linewidth]{Speicher.jpg}
...
\\\\\textbf{Symmetrische Chiffren}(TODO Schneier, S.4, 223, 249)\\ 
...
Diese Chiffren wiederum teilen sich in die zwei Gruppen Strom- und Blockchiffren. Die häufigste Form der symmetrischen Algorithmen sind die Blockchiffren. Eine Blockchiffre erhält als Eingabe einen geheimen Schlüssel sowie eine Nachricht in Blocklänge (je nach Chiffre unterschiedlich) und gibt ein Chiffrat von Blocklänge aus. Hierdurch entstehen einige Probleme für Ihre Verwendung. Bei Nachrichten die größer oder kleiner der Blocklänge sind müssen die Nachrichten entweder aufgefüllt werden oder zerteilt. Das Auffüllen wiederum sorgt dafür, dass ein Angreifer Analysen über den Verschlüsselten Klartext anstellen kann, da er vermuten kann mit was und wieviel der Nachricht aufgefüllt wurden. Daher wurden für dieses Auffüllen Komplexe Mechanismen entworfen, das sogenannte Padding. Stromchiffren dagegen arbeiten auf Bitebene und sind Zeitabhängig. Das bedeutet, dass ein Bit unterschiedlich behandelt wird, je nachdem an welcher Stelle es im Chiffrat vorkommt. Am besten kann man sich eine Stromchiffre als Pseudozufallszahlen Generator vorstellen (manche sind sogar aus solchen entworfen): Ein solcher Generator erzeugt nach eingabe des Keys (hier Seed genannt) einen Bitstrom der Objektiv betrachtet zufällig ist, also etwa gleichverteiltes Vorkommen von 0 und 1 hat und keine Regelmäßigkeiten aufweißt. Dieser erzeugte Bitstrom wird jetzt per XOR Operation mit dem Klartext verknüpft. Jemand der den Bitstrom entschlüsseln möchte braucht den selben Generator sowie den Seed und ist in der Lage mit der selben XOR Operation den Klartext wieder herzustellen.

Während Bruce Schneier in seinem Standardwerk Angewandte Kryptographie noch schreibt, Stromchiffren seien die beliebteren doch es könne bald ein Umbruch erfolgen, sind Blockchiffren heute deutlich verbreiteter, was sicher auch an Standards wie AES liegt. AES ist ein Verschlüsselungsstandard des National Institute of Standardisation (NIST) der US-Amerikaner und soll im Folgenden, als der häufigste Vertreter der symmetrischen Chiffren näher erläutert werden:
\\\textbf{AES}\\ TODO Citation
...
\\\\\textbf{Betriebsmodi}\\ TODO Schneier, S.223ff
Für Blockchiffren (und teilweise auch für Stromchiffren) wurden im Laufe der Jahre verschiedene sog. Betriebsmodi entwickelt. Ein Betriebsmodus stellt die Art und Weise da in der eine Blockchiffre verwendet wird. Sie wurden eingeführt, da die Verwendung eine Blockchiffre in ihrer reinen Form problematisch werden kann. Zum einen ist dies, wie oben erwähnt, die Tatsache, dass die Eingabe der Blocklänge entsprechen muss und ein Padding gefahren für die Sicherheit mitbringen kann. Hinzu kommt, dass eine Chiffre bei gleichem Schlüssel und gleicher Eingabe immer das selbe Chiffrat erzeugt, was kryptographische Analysen erleichtert. Der einfachste denkbare Betriebsmodus nennt sich Electronic Codebook Mode. Hier wird die Nachricht in Blöcke von Blocklänge zerteilt und jeweils einzeln Verschlüsselt. Dieser löst keines der oben gennante Probleme, ermöglicht aber die Nachricht paralel zu Verschlüsseln, also einen Geschwindigkeitsvorteil auf rechnstarken Systemen zu erzeugen. Weitere Betriebsmodi erzeugen aus einer zugrundeliegenden Blockchiffre eine Stromchiffre oder machen Chiffratblöcke von Vorangegangenen abhängig um dafür zu sorgen, dass die Eindeutigkeit von Chiffratblöcken aufgehoben wird. Im Folgenden soll einer dieser Betriebsmodi, nämlich der Counter Mode, näher erläutert werden.
\\\textbf{Counter Mode}\\(TODO schneier 243f)
